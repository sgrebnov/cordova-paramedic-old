...
TestsRunner.prototype.prepareAppToRunTarget = function(target) {
    ...
    return Q.Promise(function(resolve, reject) {
        ...
        if (me.server.tunneledUrl || me.server.externalServerUrl) {
            me.writeMedicLogUrl(me.server.tunneledUrl || me.server.externalServerUrl + ":" + me.server.port);
        } else {
...
TestsRunner.prototype.checkPlatform = function(target) {
    return Q.Promise(function(resolve, reject) {
        ...
        var result = shell.exec('cordova requirements ' + target.platformId);
        if (result.code !== 0) {
