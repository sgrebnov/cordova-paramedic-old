...
TestsRunner.prototype.prepareAppToRunTarget = function(target) {
    ...
    return Q.Promise(function(resolve, reject) {
        ...
        if (me.server.externalServerUrl) {
            me.writeMedicLogUrl(me.server.externalServerUrl + ":" + me.server.port);
        } else {
...
TestsRunner.prototype.checkPlatform = function(target) {
    return Q.Promise(function(resolve, reject) {
        logger.normal("cordova-paramedic: checking requirements for platform " + target.platform);
        var result = shell.exec('cordova requirements ' + target.platform);
        if (result.code != 0) {
            reject(new Error('Failed to check requirements for this platform! Skipping...'));
        } else resolve();
    });
}
