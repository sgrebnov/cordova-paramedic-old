...
ParamedicRunner.prototype = {
                ...
                switch(target.platformId) {
                    ...
                    case "wp8" :
                        ...
                        request.get('http://google.com/', function(e, res, data) {
                            if(e) {
                                logger.error("failed to detect ip address");
                                self.cleanUpAndExitWithCode(1);
                            }
                            else {
                                var ip = res.req.connection.localAddress ||
                                         res.req.socket.localAddress;
                                logger.normal("Using ip : " + ip);
    ...
    startServer: function() {
        ...
        return Q.promise(function (resolve, reject) {
            logger.info("cordova-paramedic: starting local medic server " + this.platformId);
            http.createServer(runner.requestListener.bind(runner))
